...
export const postToInstagram = tool(
    ...
    },
    {
        ...
        description: "Post a caption and images or vedios URL to Instagram",
        schema: z.object({
...
export const searchVideosTool = tool(
    async ({ query }) => {
        ...
        const videoUrls = response.data.videos.map((video) => {
            // video_files has multiple resolutions
            const sortedFiles = video.video_files.sort(
            (a, b) => b.width - a.width // largest width first
            );
            return sortedFiles[0].link; // return the largest resolution
        });

        // console.log("Pexels video URLs:", videoUrls);
        return videoUrls;
        } catch (error) {
        console.error("Pexels API error:", error);
        return [];
    ...
    {
        name: "search_videos",
        description: "Finds video links from Pexels related to a topic.",
        schema: z.object({
        query: z.string().describe("The topic to search videos for"),
        }),
    }
);
...
// the old one go to calendar-tools file
export const showInstaCalendar = tool(
    ...
    },
    {
        ...
        -if the user asking for an update you shoulde reopen it directly.
        `,
        schema: z.object({
        query: z
            .enum(["open", "close"])
